...
class LandingPageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return {
            "counter_bags": Donation.objects.aggregate(sum_bag=Sum("quantity"))["sum_bag"]
            if Donation.objects.all()
            else 0,
            "counter_organization": Donation.objects.values("institution").count(),
...
class AddDonationView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.context["categories"] = serialize("json", Category.objects.all())
        self.context["institutions"] = serialize(
            "json", Institution.objects.all(), fields=["name", "categories"]
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "" in [val for key, val in data.items()]:
            add_message(
                request, ERROR, "Uzupełnij wymagane pola. Uwagi dla kuriera nie są wymagane."
            )
        ...
        if len(data["phone_number"]) != 9:
            add_message(request, ERROR, "Pole z numerem telefonu wymaga podania 9 cyfr.")
        ...
        try:
            ...
            add_message(request, ERROR, "Błędy w formularzu. Wypełnij jeszcze raz.")
        ...
        if len(data["address"]) > 128:
            add_message(request, ERROR, "Adres jest za długi (max. 128 znaków).")

        if len(data["city"]) > 64:
            add_message(request, ERROR, "Za długa nazwa miasta (max. 64 znaki).")

        if len(data["pick_up_comment"]) > 255:
            add_message(request, ERROR, "Za dużo uwag dla kuriera (max. 255 znaków).")

        if (
            len(data["pick_up_date"]) == 10
            and data["pick_up_date"][4] == "-"
            and data["pick_up_date"][7] == "-"
        ):
        ...
        try:
            data["institution"] = get_object_or_404(Institution, pk=data["institution"])
        except (
            Http404,
            ValueError,
        ):
