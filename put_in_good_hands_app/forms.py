...
from put_in_good_hands_app.models import Donation
...
class RegisterForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email):
            self.add_error("email", "Konto o podanym emailu już istnieje !")
        return email
...
class DonationForm3(forms.ModelForm):
    class Meta:
        model = Donation
        fields = ("institution",)
        widgets = {"institution": forms.RadioSelect}
...
class DonationForm4(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "address": forms.TextInput(attrs={"maxlength": 128}),
            "city": forms.TextInput(attrs={"maxlength": 64}),
            "zip_code": forms.NumberInput(attrs={"pattern": "[0-9]{2}-[0-9]{3}"}),
            "phone_number": forms.NumberInput(
                attrs={"type": "tel", "pattern": "[0-9]{3}[0-9]{3}[0-9]{3}"}
            ),
            "pick_up_date": forms.SelectDateWidget,
            "pick_up_time": forms.TimeInput,
            "pick_up_comment": forms.Textarea(attrs={"maxlength": 255}),
        }
